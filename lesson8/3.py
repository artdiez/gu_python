...
class ListValidate:
    ...
    def validate(self):
        while True:
            data = input('Введите элемент списка (число). Для прекращение ввода введите "stop": ')
            if data == 'stop':
                break
            else:
                try:
                    self.list.append(int(data))
                except Exception:
                    print('Вы ввели текст, а не число!')
                    continue
        return self.list

list0 = ListValidate()
print(list0.validate())